...
describe('should', () => {
	...
	it<TestContext>('should transition to the next state', ({ machine }) => {
		...
		expect(handler.mock.calls[0]![0].transition)
			.toEqual({
				source: 'idle',
				event: 'start',
				target: 'loading',
		...
		expect(handler.mock.calls[1]![0].transition)
			.toEqual({
				source: 'loading',
				event: 'done',
				target: 'finished',
	...
	it<TestContext>('should filter transition handler', ({ machine }) => {
		...
		expect(handler1)
			.toHaveBeenCalledTimes(1)
		expect(handler1.mock.calls[0]![0].transition)
			.toEqual({
				source: 'idle',
				event: 'start',
				target: 'loading',
		...
		expect(handler2)
			.toHaveBeenCalledTimes(1)
		expect(handler2.mock.calls[0]![0].transition)
			.toEqual({
				source: 'idle',
				event: 'start',
				target: 'loading',
		...
		expect(handler3)
			.toHaveBeenCalledTimes(1)
		expect(handler3.mock.calls[0]![0].transition)
			.toEqual({
				source: 'idle',
				event: 'start',
				target: 'loading',
	...
	it<TestContext>('should destroy', ({ machine }) => {
		...
		expect(() => machine.currentState)
			.toThrow()
		expect(() => machine.onBeforeDestroyed(() => {}))
			.toThrow()
		expect(() => machine.onAfterDestroyed(() => {}))
			.toThrow()
		expect(() => machine.send('start'))
			.toThrow()
		expect(() => machine.onTransition(() => {}))
			.toThrow()
